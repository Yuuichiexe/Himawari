...
from Himawari.modules.gifkang import convert_gif
...
def makepack_internal(
    ...
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton(text="𝚅ɪᴇᴡ 𝙿ᴀᴄᴋ", url=f"{packname}")]]
    )
    try:
        extra_version = ""
        if packnum > 0:
        ...
        if png_sticker:
            success = context.bot.create_new_sticker_set(
                user.id,
                packname,
                f"{name}s Pack :: @Esdeath_probot" + extra_version,
            ...
            msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                text="𝚂ᴛᴀʀᴛ", url=f"t.me/{context.bot.username}",
                            ),
                        ],
                    ],
                ),
