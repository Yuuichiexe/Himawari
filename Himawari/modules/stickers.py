...
def kang(update, context):
    ...
    if msg.reply_to_message:
        ...
        adding_process = msg.reply_text(
            "<b>Your sticker will be added in few seconds, please wait... \n\n Note : if you are using kang first time then your pack is ready. </b>",
            parse_mode=ParseMode.HTML,
        )
...
def makepack_internal(
    ...
    try:
        ...
        if png_sticker:
            sticker_pack_name = (
                f"{name}'s Pack (@{context.bot.username})" + extra_version
            )
...
•  /tiny*:* Reply to a sticker or image to make it smaller
"""
